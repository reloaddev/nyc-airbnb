...
def get_pie_chart():
    pie_chart = px.pie(neighbourhood_group_data, names='neighbourhood_group', values='percentage',
...
def update_pie_chart(click_data, n_clicks, figure):
    # Back button clicked
    if n_clicks > 0:
        fig = px.pie(neighbourhood_group_data, names='neighbourhood_group', values='percentage',
                     title="Percentage of Listings by Neighbourhood Group",
                     hover_data=['listings_count'],
                     labels={'percentage': '% Listings'},
                     color='neighbourhood_group',
                     color_discrete_map=color_mapping)  # Apply custom color mapping
        fig.update_traces(textposition='outside', textinfo='percent+label')  # Move percentage outside
        return fig, {'display': 'none'}
 
    # Click on a neighbourhood group
    if click_data:
        ...
        fig = px.pie(room_type_data, names='room_type', values='percentage',
                     title=f"Percentage of Room Types in {selected_group}",
                     hover_data=['count'],
                     labels={'percentage': '% Room Types'},
                     color='room_type',  # Apply color based on room type
                     color_discrete_map=color_mapping)
        fig.update_traces(textposition='outside', textinfo='percent+label')  # Move percentage outside
